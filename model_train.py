...
from engine_original import GeneralMovingAverage, get_dataset, get_text_features, train, evaluate_all

device = torch.device("cuda:4" if torch.cuda.is_available() else "cpu")


def main(args: argparse.Namespace):
    print(f"Running on {args.data}:")
    ...
    best_pth=logger.get_checkpoint_path(args.pth)
    if(os.path.isfile(best_pth)):
        print("Loading from "+best_pth)
        classifier.load_state_dict(torch.load(logger.get_checkpoint_path(args.pth)))
    classifier.eval()
    ...
    if args.phase == "train":
        ...
        for epoch in range(args.epochs):
            ...
            # torch.save(classifier.state_dict(), logger.get_checkpoint_path('latest'+"-"+args.pth.split("-")[-1]))
            if val_acc1 > best_val_acc1:
                print("Saving the best")
                # shutil.copy(logger.get_checkpoint_path('latest'+"-"+args.pth.split("-")[-1]), logger.get_checkpoint_path(args.pth))
                torch.save(classifier.state_dict(), logger.get_checkpoint_path(args.pth))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pth', '--model-path', metavar='DATA', default='best')
