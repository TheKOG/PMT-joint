...
def get_dataset_source(args):
    if args.task == "domain_shift":
        # load domainbed data
        train_datasets, val_datasets, test_datasets, class_names = \
            converter_domainbed.get_domainbed_datasets(dataset_name=args.source, root=args.root, targets=args.targets, holdout=0.2)
        train_class_names = class_names
        train_iter = {
        ...
        # load domainbed data
        train_datasets, val_datasets, test_datasets, open_datasets, base_class_names, open_class_names = \
            converter_domainbed.get_domainbed_datasets(dataset_name=args.source, root=args.root, targets=args.targets, holdout=0.2, seed=args.seed, open_ratio=0.5)
        train_class_names = base_class_names
        train_iter = {
                "iter":converter_domainbed.get_forever_iter(train_datasets, args.batch_size, num_workers=args.workers),
                "class_names": class_names
...
def prompts_train(train_iter: ForeverDataIterator, model:CustomCLIP, moving_avg_model: GeneralMovingAverage,
          optimizer, lr_scheduler, epoch: int, args, writer: TensorboardWriter, device):
    batch_time = AverageMeter('Time', ':4.2f')
    data_time = AverageMeter('Data', ':3.1f')
    losses = AverageMeter('Loss', ':3.2f')
...
def pmt_train(train_iter: ForeverDataIterator, model:CustomCLIP, moving_avg_model: GeneralMovingAverage,
...
def validate(val_loader, model:CustomCLIP, text_features, args, device, shift=0) -> float:
    ...
    return top1.avg
def validate_pmp(val_loader, model:CustomCLIP, args, device, shift=0) -> float:
    batch_time = AverageMeter('Time', ':6.3f')
    top1 = AverageMeter('Acc@1', ':6.2f')
    progress = ProgressMeter(
        len(val_loader),
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # measure accuracy and record loss
            # pdb.set_trace()
            output_similarity = model(images,prefixes,suffixes)
...
def validate_pmt(val_loader, model:CustomCLIP, args, device, shift=0,threshold=0.92,train=True) -> float:
    # switch to evaluate mode
    model.eval()
    prefixes,suffixes=None,None
    if type(val_loader)==dict:
...
def evaluate_all_pmp(model:CustomCLIP, val_loader, test_loaders, args, writer, device):
    print("Evaluate on validation set...")
    val_acc1 = validate_pmp(val_loader, model, args, device)
    writer.write_eval_values({"Acc@1": val_acc1}, prefix="val")
    # pdb.set_trace()
    for test_loader in test_loaders:
        split_name = test_loader["name"]
        print(f"Evaluate on {split_name} set...")
        validate_pmp(test_loader,model, args, device)
        writer.write_eval_values({"Acc@1": val_acc1}, prefix=split_name)

    return val_acc1

def evaluate_all_pmt(model:CustomCLIP, val_loader, test_loaders, args, writer, device,threshold=0.922,train=True):
    torch.autograd.set_detect_anomaly(True)
    print("Evaluate on validation set...")
    # pdb.set_trace()
    val_acc1 = validate_pmt(val_loader, model, args, device,threshold=threshold,train=train)
    writer.write_eval_values({"Acc@1": val_acc1}, prefix="val")
    for test_loader in test_loaders:
        split_name = test_loader["name"]
        print(f"Evaluate on {split_name} set...")
        validate_pmt(test_loader,model, args, device,threshold=threshold,train=train)
        writer.write_eval_values({"Acc@1": val_acc1}, prefix=split_name)
