...
def setup_cfg(args):
    # 读取YAML文件
    with open(args.pmt_cfg_file, 'r') as f:
...
def main(args: argparse.Namespace):
    ...
    clip_pmt=CustomCLIP(clip_model=clip_model,classnames=train_class_names,cfg=setup_cfg(args),device=device)

    # create model
    print("=> using pre-trained model '{}'".format(args.arch))
    classifier = clip_model.visual
    
    best_pth=logger.get_checkpoint_path('best-PACS')
    if(os.path.isfile(best_pth)):
        classifier.load_state_dict(torch.load(logger.get_checkpoint_path('best-PACS')))
    ...
    for test_loader in test_loaders:
        test_loader["prefixes"],test_loader["suffixes"]=clip_pmt.prompt_learner.get_pre_suf(test_loader["class_names"],clip_model)
    ...
    if args.phase == "train":
        ...
        best_val_acc1 = evaluate_all_pmt(clip_pmt, val_loader, test_loaders, args, writer, device,threshold=threshold,train=False)
        # best_val_acc1=78.525
        # start training
        for epoch in range(args.epochs):
            print("Learning rate: {:.4e}".format(lr_scheduler.get_last_lr()[0]))
            # train for one epoch
            # pmt_train(train_iter, clip_pmt, bma_classifier, optimizer, lr_scheduler, epoch, args, writer, device,threshold=threshold)

            # evaluate all
            val_acc1 = evaluate_all_pmt(clip_pmt, val_loader, test_loaders, args, writer, device,threshold=threshold,train=True)
            continue
            ...
            if val_acc1 > best_val_acc1:
                ...
                clip_pmt.prompt_learner.Save()

        print("Training completed.")

    # print("Evaluate best model:")
    # evaluate_all_pmt(clip_pmt, val_loader, test_loaders, args, writer, device)
